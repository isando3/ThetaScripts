...
def binFile(rerror, filename, xtitle, backgrounds):
    ...
    for key in h_bkg:
       ...
       # h_data[key] = h_data[key].Rebin(len(binning)-1, h_data[key].GetName(), binning)
        ...
        max =  h_bkg[key].GetMaximum()
        min = h_bkg[key].GetBinContent(h_bkg[key].GetMinimumBin())

        ##when data is present 
        #h_data[key].GetYaxis().SetRangeUser(0.5*min,max*1.8)
        ...
        ###
        ### background and signal only
        h_bkg[key].GetYaxis().SetRangeUser(0.5*min,max*1.8)

        h_bkg[key].GetYaxis().SetLabelSize(0.05)
        h_bkg[key].GetYaxis().SetTitleSize(0.05)
        h_bkg[key].GetYaxis().SetTitle('event yield')
        h_bkg[key].GetXaxis().SetLabelSize(0.05)
        h_bkg[key].GetXaxis().SetTitleSize(0.05)
       ...
       # h_bkg[key].Draw('samehist')
        ...
        labellumi.SetHeader('L = 5.0 fb^{-1}')
        labellumi.Draw()
...
binFile(0.3, 'thetainput.root', 'M_{t#bar{t}} [GeV/c^{2}]', ['ttbar', 'wjets'])
